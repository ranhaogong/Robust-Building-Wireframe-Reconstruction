...
class EdgeAttentionNet(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        point_coords = batch_dict['keypoint'][:, 1:4]  # 提取坐标用于距离计算
        ...
        # 将三者特征进行拼接或累加
        edge_fea = edge_fea + global_fea + att_fea
        ...
        if self.training:
            ...
            self.train_dict['pair_coords1'] = point_coords[pair_idx1]  # 保存点对坐标
            self.train_dict['pair_coords2'] = point_coords[pair_idx2]
        return batch_dict

    def loss(self, loss_dict, disp_dict):
        pred_cls = self.train_dict['edge_pred']
        label_cls = self.train_dict['label']
        pair_coords1 = self.train_dict['pair_coords1']  # (M, 3)
        pair_coords2 = self.train_dict['pair_coords2']  # (M, 3)

        # 分类损失
        cls_loss = self.get_cls_loss(pred_cls, label_cls, self.loss_weight['cls_weight'])

        # 物理损失（距离约束）
        distances = torch.norm(pair_coords1 - pair_coords2, dim=-1)  # (M,)
        pred_prob = torch.sigmoid(pred_cls.squeeze(-1))  # (M,)
        distance_loss = torch.mean(pred_prob * distances)  # 正样本距离较小
        physical_loss = distance_loss * self.loss_weight.get('distance_weight', 0.1)

        # 总损失
        loss = cls_loss + physical_loss
        ...
        num_positives = torch.sum(label_cls == 1)
        num_correct = torch.sum((pred == label_cls) & (label_cls == 1))
        acc = num_correct.item() / max(num_positives.item(), 1)  # 避免除以零
